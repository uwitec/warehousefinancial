	...
	<script type="text/javascript">
		Ext.onReady(function(){
			...
			var dataStore = new Ext.data.JsonStore({//数据存储
		        ...
		        url:contextPath+'/jxc/FywhAction.do?method=getAllFywh',//contextPath+
				fields:['id','xfzmc','xfsj','fy','jlsj','jlrmc','memo']//实体属性名称
			});
			//费用类型
			var fylxStore =	new Ext.data.JsonStore({
				data:[{fylx:'0',lxmc:'人费用'},{fylx:'1',lxmc:'车费用'}],
				fields:['fylx','lxmc']
			});
			//消费者
			var xfStore = new Ext.data.JsonStore({
				url:contextPath+"/jxc/FywhAction.do?method=getAllXfdxByJoin",
				//autoLoad:true,
				fields:["xfid","xfmc"]
			});
						...
						handler:function(){
							...
							if(aqf.form.isValid()){
								aqf.form.doAction('submit',{
									url:contextPath+'/jxc/FywhAction.do?method=addFywh',
									method:'post',
									waitTitle:'提示信息',                                                    
			                        waitMsg:'数据更新中，请稍候...',
								...
								items:[
									{
										...
										listeners:{
											'select':function(cob,r,ind){
												...
												Ext.apply(xfStore.baseParams,{column:'sid',sid:r.get('fylx')});
												xfStore.reload();
											}
										}
									},{
			...
			getSaveBtn().on('click',function(){
				...
				Ext.Ajax.request({
					method: 'POST',
					url: contextPath+'/jxc/FywhAction.do?method=updateFywh',
				...
				Ext.Msg.confirm("删除提示","是否确认删除你选中的费用信息？",function(btn,text){
					if( btn == 'yes'){
						...
						Ext.lib.Ajax.request(
							'POST',
							contextPath+'/jxc/FywhAction.do?method=deleteFywh&id='+id,
							{
								success:function(response){
			...
			setNavigate("费用管理 >> 费用信息维护 ","本模块维护费用信息");
			dataStore.load();
