	...
	<script type="text/javascript">
		Ext.onReady(function(){
			...
			var drbStore = new Ext.data.JsonStore({
				url:'<c:url value="/system/lxdmglAction.do?method=getLxdmBySjjc&dmjc=DRB"/>',
				fields:["dmjc","dmmc"]
			});
			var tableStore = new Ext.data.JsonStore({
				autoDestroy: true,
				pruneModifiedRecords:true,
		        url: '<c:url value="/common/dataImportAction.do?method=getTableColumns"/>',
			...
			var dataGrid = new Ext.grid.GridPanel({
				...
				tbar:{
					...
					items:[
						{
							xtype:'button',
							icon:'<c:url value="/imgs/import.gif"/>',
							text:'读取源文件',
							handler:function(){
								imporWin.show();
							}
			   	...
			   	items: [
			    	{
						...
						store:new Ext.data.JsonStore({
							data:[{sid:true,sname:'是'},{sid:false,sname:'否'}],
							fields:["sid","sname"]
						})			
					 },{
					 	xtype:'hidden',
				...
				buttons:[
					{
						...
						handler:function(){
							...
							if(value.substring(value.length-4,value.length)!=".xls"
								&&value.slice(-5)!='.xlsx'){
								Ext.Msg.alert("提示","只能导入后缀为xls或xlsx的文件，请重新选择数据源文件！");
							...
							if (importPanel.getForm().isValid()) {
								importPanel.getForm().submit({
									...
									isUpload:true,
									method: 'POST',
									url:'<c:url value="/common/dataImportAction.do?method=loadColumnHeader"/>',
						...
						items:[
							{
								...
								items:[
									{
										width:80,
										xtype:'button',
										icon:'<c:url value="/imgs/to_left.gif"/>',
										text:'字段匹配',
										handler:function(){
								...
								items:[{
									width:80,
									xtype:'button',
									icon:'<:url value="/imgs/delete.png"/>',
									text:'撤销匹配',
									handler:function(){
										var ts = tableSm.getSelections();
								...
								items:[{
									...
									xtype:'button',
									text:'自动匹配',
									icon:'<c:url value="/imgs/update.gif"/>',
									handler:function(){
								...
								items:[{
									...
									xtype:'button',
									text:'执行导入',
									icon:'<c:url value="/imgs/save.gif"/>',
									handler:function(){								
										var value = Ext.getCmp('importuser_value_text').getValue();
			...
			function dataImport(){
				if (importPanel.getForm().isValid()) {
					...
					importPanel.getForm().submit({
						...
						isUpload:true,
						method: 'POST',
						url:'<c:url value="/common/dataImportAction.do?method=dataValidate"/>',
						success: function(form, action){			
							imporWin.hide();
