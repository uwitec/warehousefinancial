package com.wfms.service.common.dataimport;
...
import com.wfms.webapp.common.dataimport.attribute.ActionReport;
import com.wfms.webapp.common.dataimport.attribute.ImportField;
import com.wfms.webapp.common.dataimport.attribute.LxdmType;
import com.wfms.webapp.common.dataimport.command.DataImportForm;
import com.wfms.webapp.common.dataimport.util.DataImportValidater;
...
public class DefaultDataImportHandler extends DataImportHandler {
	...
	public ActionReport validateRows(Map<String, String> fieldValueMap,
		...
		for (ColumnEntity colEntity : super.colEntList) {
			...
			{
				if(drgz!=null && drgz.getWbzdlx()!= null && DataImportConstant.NUMBER
								.equals(drgz.getWbzdlx()))
				{
			...
			//验证字符长度，忽略有类型代码转换的情况
			else if (DataImportConstant.VARCHAR2.equals(colEntity
					.getDataType()) || DataImportConstant.VARCHAR.
					equals(colEntity.getDataType()) || DataImportConstant.CHAR.
					equals(colEntity.getDataType())) {
				//没有代码转换的情况
				if(drgz == null)
				{
					int dataLen = colEntity.getDataLength();
					if (!DataImportValidater.isValidateVarchar2Len(colVal, dataLen)) {
						errorCount++;
			...
			if (drgz != null) {
				...
				// 直接转换成对应的值
				if (drgzMap.get(zdmc+colVal) == null 
						&& drgz.getCkb()==null) {
					errorCount++;
					errorMsgs.append(comment + "字段值必须录入系统所指定的值 ");
						...
						&& drgz.getCkb()==null)
				{
					fieldValueMap.put(zdmc, drgzMap.get(zdmc+colVal).getNbsjz());
				}
				// 根据类型代码表的代码值进行转换
						...
						&& DataImportConstant.LXDMB.equalsIgnoreCase(drgz
								.getCkb())) {
					LxdmEntity lxdm = lxdmDao.getLxdmBySuperDmAndDmmc(drgz
							.getFllxdm(), fieldValueMap.get(zdmc));
					if ((lxdm == null || "".equals(lxdm))
						&& DataImportConstant.FQZFK.equals(colEntity.getNullable())) {
				...
				// 非类型代码表的代码值进行转换
					...
					} else if(DataImportConstant.FQZFK.equals(colEntity.getNullable())) {
						errorCount++;
	...
	public ActionReport commonImport(String tableName, String[] columns,
		...
		for (int i = 2; i < rowcount; i++) {
			...
			try {
				...
				if (pkColumns == null && (pkColsList==null || pkColsList.size()==0)) {
					exsitsRs = false;
					...
					{  //根据数据库唯一索引判断记录是否存在
						...
						{
							String[] pkColValues = getColumnValues(pkCols, fieldValueMap);
							exsitsRs = importDao.exsitsRsByPks(tableName, pkCols,
									pkColValues);
							if(exsitsRs)
							{
								pkColumns = pkCols;
				...
				if (!exsitsRs) {
					if (insertPermit) {
						// 插入记录
						sqlList.add(getAutoInsertSql(tableName, columns, values));
						//importDao.insertWithAutoID(tableName, columns, values);
					} else {
						continue;
					}
