package com.wfms.webapp.common.datastat.util;
...
import com.wfms.dao.common.datastat.CommonStatDAO;
import com.wfms.dao.common.datastat.IStatDefineDao;
import com.wfms.model.common.datastat.StatDefineEntity;
import com.wfms.webapp.common.datastat.attribute.FusionChart;
import com.wfms.webapp.common.datastat.attribute.FusionUnit;
import com.wfms.webapp.common.datastat.attribute.StatConditionBean;
import com.wfms.webapp.common.datastat.attribute.StatResultSetBean;
import com.wfms.webapp.common.datastat.attribute.StatUnit;
import com.wfms.webapp.common.datastat.attribute.v3.Category;
import com.wfms.webapp.common.datastat.attribute.v3.CategoryUnit;
import com.wfms.webapp.common.datastat.attribute.v3.Chart;
import com.wfms.webapp.common.datastat.attribute.v3.Data;
import com.wfms.webapp.common.datastat.attribute.v3.DataSet;
...
public class StatUtil {
	...
	public void initStatUtil(String statType, String columnKeyType, String tableName,
			String condition, List<String> columnKeys) throws ChartCreateException {
		this.columnKeys = columnKeys;
		...
		/*if (columnKeys == null || columnKeys.size() == 0) {
			throw new ChartCreateException("无此统计对象的数据", 56);
		}
		if (rowKeys == null) {
			throw new ChartCreateException("此分类无效", 59);
		}
		*/
	...
	public void initStatUtil(StatConditionBean condBean) throws ChartCreateException {
	...
	public void initStatUtil(StatResultSetBean rsBean,StatConditionBean condBean) throws ChartCreateException {
		this.rsBean = rsBean;
		this.statCondBean = condBean;
		if (!dataStatDao.initStatResultSetBean(this,rsBean,condBean)) {
			throw new ChartCreateException("统计失败", 85);
	...
	private void totalizeData(String calcType) {
		...
		if (calcType != null && calcType.equals("percent")) {
			for (int m = 0; m < data.length; m++) {
				retData[m][retData[m].length - 1] = total==0?0:retData[m][retData[m].length - 1]
						/ total;
			}
			for (int n = 0; n < data[0].length; n++) {
				retData[retData.length - 1][n] = total==0?0:retData[retData.length - 1][n]
						/ total;
			}
	...
	public void createBarChart(HttpServletRequest request, String title) {
		...
		// 生成柱状图
		JFreeChart chart = MyChartFactory.getBarChart(data, rows, columns,generalTitle);
	...
	public FusionChart createFusionBar(String title) {
		...
		for(int i=0;i<rows.length;i++)
		{
			//unit.getCategorySet().add(new StatUnit(rows[i],rsBean.getRs().get(i).get(0)));
			for(int j=0;j<columns.length;j++)
			{
				unit.getCategorySet().add(new StatUnit(rows[i],rsBean.getRs().get(i).get(j)));
				unit.getSet().add(new StatUnit(columns[j],rsBean.getRs().get(i).get(j)));
			}
		}
		chart.setGraph(unit);
		return chart;
	...
	public com.wfms.webapp.common.datastat.attribute.v3.FusionChart createFusionChart(String title) {
		// 生成分析图
		initData();
		...
		com.wfms.webapp.common.datastat.attribute.v3.FusionChart fusionChart = new com.wfms.webapp.common.datastat.attribute.v3.FusionChart();
	...
	public StatDefineEntity getStatDefine(String tableName)
	{
