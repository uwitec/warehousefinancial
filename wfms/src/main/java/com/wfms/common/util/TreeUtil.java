...
public class TreeUtil {
	...
	public static List<TreeNode> initTree(String contextPath,
			List<ModuleGenInfo> list, boolean... checked) {
		if (list == null) {
			return null;
		}
	...
	private static TreeNode getTree(String contextPath, ModuleGenInfo gnmk,
		...
		if (gnmk.isLeaf()) {
			tree.setUrl(contextPath + gnmk.getForwardpage());
		} else {
			tree.setUrl("javascript:void(0);");
		}
		if (checked != null && checked.length > 0) {
			tree.setChecked(checked[0]);
		}
		return tree;
	}
	...
	private static TreeNode getChildrenNode(String contextPath,
			List<ModuleGenInfo> list, ModuleGenInfo gnmk, boolean... checked) {
		TreeNode tree = getTree(contextPath, gnmk);
		// 递归标记
		for (int i = 0; i < list.size(); i++) {
			if (list.get(i).getParentid().equals(gnmk.getId())) {
				// 递归调用
				TreeNode rstree = getChildrenNode(contextPath, list,
						list.get(i), checked);
				// 递归标记
